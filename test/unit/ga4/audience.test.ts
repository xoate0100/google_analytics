...
describe("GA4 Audience Tools", () => {
  ...
  describe("registerGA4Tools", () => {
    it("should register ga4.audience.list tool", () => {
      registerGA4Tools({
      ...
      expect(registeredTools.has("ga4.audience.list")).toBe(true);
    });

    it("should register ga4.audience.get tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.audience.get")).toBe(true);
    });

    it("should register ga4.audience.upsert tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.audience.upsert")).toBe(true);
    });

    it("should register ga4.audience.delete tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.audience.delete")).toBe(true);
    });
  });

  describe("ga4.audience.list handler", () => {
    it("should validate request schema", async () => {
      ...
      const tool = registeredTools.get("ga4.audience.list") as {
    ...
    it("should call Analytics Admin API to list audiences", async () => {
          ...
          audiences: {
            list: vi.fn().mockResolvedValue({
              data: {
                audiences: [
                  {
                    name: "properties/123456789/audiences/987654321",
  ...
  describe("ga4.audience.get handler", () => {
    it("should validate request schema", async () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
      ...
      const tool = registeredTools.get("ga4.audience.get") as {
        handler: (args: unknown) => Promise<unknown>;
      };
    ...
    it("should call Analytics Admin API to get audience", async () => {
      const mockAdminClient = {
        properties: {
          audiences: {
      ...
      expect(mockGA4Client.checkRateLimit).toHaveBeenCalled();
      expect(result).toHaveProperty("displayName", "Test Audience");
    });
  });
});
