...
describe("GA4 Event Definition Tools", () => {
  ...
  describe("registerGA4Tools", () => {
    it("should register ga4.event.list tool", () => {
      registerGA4Tools({
      ...
      expect(registeredTools.has("ga4.event.list")).toBe(true);
    });

    it("should register ga4.event.get tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.event.get")).toBe(true);
    });

    it("should register ga4.event.upsert tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.event.upsert")).toBe(true);
    });
  });

  describe("ga4.event.list handler", () => {
    it("should validate request schema", async () => {
      ...
      const tool = registeredTools.get("ga4.event.list") as {
    ...
    it("should call Analytics Admin API to list events", async () => {
          ...
          eventCreateRules: {
            list: vi.fn().mockResolvedValue({
              data: {
                eventCreateRules: [
                  {
                    name: "properties/123456789/eventCreateRules/purchase",
                    eventName: "purchase",
                    createEvent: true,
                  },
                ],
  ...
  describe("ga4.event.get handler", () => {
    it("should validate request schema", async () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
      ...
      const tool = registeredTools.get("ga4.event.get") as {
        handler: (args: unknown) => Promise<unknown>;
      };
    ...
    it("should call Analytics Admin API to get event", async () => {
      const mockAdminClient = {
        properties: {
          eventCreateRules: {
