...
describe("GA4 Data Retention Tools", () => {
  ...
  describe("registerGA4Tools", () => {
    it("should register ga4.property.dataRetention.get tool", () => {
      registerGA4Tools({
      ...
      expect(registeredTools.has("ga4.property.dataRetention.get")).toBe(true);
    });

    it("should register ga4.property.dataRetention.update tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.property.dataRetention.update")).toBe(true);
    });
  });

  describe("ga4.property.dataRetention.get handler", () => {
    it("should validate request schema", async () => {
      ...
      const tool = registeredTools.get("ga4.property.dataRetention.get") as {
    ...
    it("should check rate limit before making request", async () => {
      const mockAdminClient = {
        properties: {
          getDataRetentionSettings: vi.fn().mockResolvedValue({
            data: {
      ...
      expect(checkRateLimitCalls.some((call) => call[0] === "ga4" && call[1] === "getDataRetentionSettings")).toBe(true);
    });
  });

  describe("ga4.property.dataRetention.update handler", () => {
    it("should validate request schema", async () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
      ...
      const tool = registeredTools.get("ga4.property.dataRetention.update") as {
        handler: (args: unknown) => Promise<unknown>;
      };

      const invalidArgs = {
        property: "invalid",
        retentionDays: "INVALID_RETENTION",
      };
    ...
    it("should check rate limit before making request", async () => {
      const mockAdminClient = {
        properties: {
          updateDataRetentionSettings: vi.fn().mockResolvedValue({
            data: {
              name: "properties/123456789/dataRetentionSettings",
      ...
      const checkRateLimitMock = checkRateLimitFn as ReturnType<typeof vi.fn>;
      const checkRateLimitCalls = checkRateLimitMock.mock.calls;
      expect(checkRateLimitCalls.some((call) => call[0] === "ga4" && call[1] === "updateDataRetentionSettings")).toBe(true);
    });
  });
});
