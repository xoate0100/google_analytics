...
describe("GA4 Conversion Tools", () => {
  ...
  describe("registerGA4Tools", () => {
    it("should register ga4.conversion.list tool", () => {
      registerGA4Tools({
      ...
      expect(registeredTools.has("ga4.conversion.list")).toBe(true);
    });

    it("should register ga4.conversion.get tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.conversion.get")).toBe(true);
    });

    it("should register ga4.conversion.upsert tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.conversion.upsert")).toBe(true);
    });

    it("should register ga4.conversion.delete tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.conversion.delete")).toBe(true);
    });
  });

  describe("ga4.conversion.list handler", () => {
    it("should validate request schema", async () => {
      ...
      const tool = registeredTools.get("ga4.conversion.list") as {
    ...
    it("should call Analytics Admin API to list conversions", async () => {
  ...
  describe("ga4.conversion.get handler", () => {
    it("should validate request schema", async () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
      ...
      const tool = registeredTools.get("ga4.conversion.get") as {
        handler: (args: unknown) => Promise<unknown>;
      };
    ...
    it("should call Analytics Admin API to get conversion", async () => {
      const mockAdminClient = {
        properties: {
          conversionEvents: {
      ...
      expect(mockGA4Client.checkRateLimit).toHaveBeenCalled();
      expect(result).toHaveProperty("eventName", "purchase");
    });
  });
});
