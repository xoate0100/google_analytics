...
describe("GA4 Custom Metric Tools", () => {
  ...
  describe("registerGA4Tools", () => {
    it("should register ga4.customMetric.list tool", () => {
      registerGA4Tools({
      ...
      expect(registeredTools.has("ga4.customMetric.list")).toBe(true);
    });

    it("should register ga4.customMetric.get tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.customMetric.get")).toBe(true);
    });

    it("should register ga4.customMetric.upsert tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.customMetric.upsert")).toBe(true);
    });

    it("should register ga4.customMetric.delete tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.customMetric.delete")).toBe(true);
    });
  });

  describe("ga4.customMetric.list handler", () => {
    it("should validate request schema", async () => {
      ...
      const tool = registeredTools.get("ga4.customMetric.list") as {
    ...
    it("should call Analytics Admin API to list custom metrics", async () => {
          ...
          customMetrics: {
            list: vi.fn().mockResolvedValue({
              data: {
                customMetrics: [
                  {
                    name: "properties/123456789/customMetrics/test_metric",
      ...
      expect(result).toHaveProperty("customMetrics");
    });
  });

  describe("ga4.customMetric.get handler", () => {
    it("should validate request schema", async () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
      ...
      const tool = registeredTools.get("ga4.customMetric.get") as {
        handler: (args: unknown) => Promise<unknown>;
      };
    ...
    it("should call Analytics Admin API to get custom metric", async () => {
      const mockAdminClient = {
        properties: {
          customMetrics: {
      ...
      expect(mockGA4Client.checkRateLimit).toHaveBeenCalled();
      expect(result).toHaveProperty("parameterName", "test_metric");
    });
  });
});
