...
describe("GA4 Event Parameter Tools", () => {
  ...
  describe("registerGA4Tools", () => {
    it("should register ga4.event.parameter.list tool", () => {
      registerGA4Tools({
      ...
      expect(registeredTools.has("ga4.event.parameter.list")).toBe(true);
    });

    it("should register ga4.event.parameter.upsert tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.event.parameter.upsert")).toBe(true);
    });

    it("should register ga4.event.parameter.delete tool", () => {
      registerGA4Tools({
        bootstrap: mockBootstrap,
        ga4Client: mockGA4Client,
        cache: mockCache,
      ...
      expect(registeredTools.has("ga4.event.parameter.delete")).toBe(true);
    });
  });

  describe("ga4.event.parameter.list handler", () => {
    it("should validate request schema", async () => {
      ...
      const tool = registeredTools.get("ga4.event.parameter.list") as {
